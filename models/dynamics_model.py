...
class DynamicsNN(torch.nn.Module):
    ...
    def transformations_to(self, device):
        if type(self.s_shift) is np.ndarray:
            self.s_shift = torch.from_numpy(self.s_shift).float()
            self.a_shift = torch.from_numpy(self.a_shift).float()
            self.s_scale = torch.from_numpy(self.s_scale).float()
            self.a_scale = torch.from_numpy(self.a_scale).float()
            self.out_shift = torch.from_numpy(self.out_shift).float()
            self.out_scale = torch.from_numpy(self.out_scale).float()
            self.mask = torch.from_numpy(self.mask)
        
        self.s_shift = self.s_shift.float().to(device)
        self.a_shift = self.a_shift.float().to(device)
        self.s_scale = self.s_scale.float().to(device)
        self.a_scale = self.a_scale.float().to(device)
        self.out_shift = self.out_shift.float().to(device)
        self.out_scale = self.out_scale.float().to(device)
...
class DynamicsModel:
    ...
    def fit_dynamics(
        self, s, a, sp, s_h, a_h, sp_h, fit_epochs, batch_size=256, max_steps=1e4, track_metrics=False, **kwargs
    ):
