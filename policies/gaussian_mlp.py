...
class MLP(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.min_log_std_val = (
            min_log_std if type(min_log_std) is np.ndarray else min_log_std * np.ones(self.action_dim)
        )
        self.max_log_std_val = (
            max_log_std if type(max_log_std) is np.ndarray else max_log_std * np.ones(self.action_dim)
        )
        ...
        # Include optional output function
        self.out_fn = out_fn
        self.weight = out_fn_weight
    ...
    def forward(self, observations):
        ...
        # TODO: this is temporal workaround for already trained policies
        if hasattr(self, "log_actions"):
            if self.log_actions:
                self.raw_outputs.append(out.to("cpu").data.numpy()[0])

            if isinstance(self.out_fn, str):
                out = torch.clamp(out, min=-1, max=1)
            elif self.out_fn is not None:
                out = self.out_fn(out * self.weight)

            if self.log_actions:
                self.outputs.append(out.to("cpu").data.numpy()[0])
    ...
    def set_transformations(self, in_shift=None, in_scale=None, out_shift=None, out_scale=None, *args, **kwargs):
    ...
    def get_action(self, observation):
        ...
        mean = self.forward(self.obs_var)
        noise = torch.Tensor(np.exp(self.log_std_val) * np.random.randn(self.action_dim))
        ...
        if hasattr(self, "log_actions"):
            if isinstance(self.out_fn, str):
                ...
                mean = self.out_fn(mean * self.weight)

        action = mean + noise

        mean = mean.to("cpu").data.numpy().ravel()
        noise = noise.to("cpu").data.numpy().ravel()
        ...
        if hasattr(self, "log_actions"):
            if isinstance(self.out_fn, str):
                ...
                action = self.out_fn(action * self.weight)

        action = action.to("cpu").data.numpy().ravel()

        # TODO: this is temporal workaround for already trained policies
        if hasattr(self, "log_actions"):
            if self.log_actions:
                self.means.append(mean)
                self.stds.append(noise)
    ...
    def mean_LL(self, observations, actions, log_std=None, *args, **kwargs):
        ...
        LL = -0.5 * torch.sum(zs**2, dim=1) + -torch.sum(log_std) + -0.5 * self.action_dim * np.log(2 * np.pi)
        return mean, LL
