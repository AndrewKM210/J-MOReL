"""
Adapted from https://github.com/aravindr93/mjrl/tree/v2/projects/morel
"""

import numpy as np
import torch


def cg_solve(f_Ax, b, x_0=None, cg_iters=10, residual_tol=1e-10, callback=None, verbose=False):
    """
    Goal: Solve Ax=b equivalent to minimizing f(x) = 1/2 x^T A x - x^T b
    Assumption: A is PSD, no damping term is used here (must be damped externally in f_Ax)
    Verbose mode works only with numpy
    """

    if type(b) is torch.Tensor:
        x = torch.zeros(b.shape[0]) if x_0 is None else x_0
        x = x.to(b.device)
        if b.dtype == torch.float16:
            x = x.half()
        r = b - f_Ax(x)
        p = r.clone()
    elif type(b) is np.ndarray:
        x = np.zeros_like(b) if x_0 is None else x_0
        r = b - f_Ax(x)
        p = r.copy()
    else:
        print("Type error in cg")

    fmtstr = "%10i %10.3g %10.3g %10.3g"
    titlestr = "%10s %10s %10s %10s"
    if verbose:
        print(titlestr % ("iter", "residual norm", "soln norm", "obj fn"))

    for i in range(cg_iters):
        if callback is not None:
            callback(x)
        if verbose:
            obj_fn = 0.5 * x.dot(f_Ax(x)) - 0.5 * b.dot(x)
            norm_x = torch.norm(x) if type(x) is torch.Tensor else np.linalg.norm(x)
            print(fmtstr % (i, r.dot(r), norm_x, obj_fn))

        rdotr = r.dot(r)
        Ap = f_Ax(p)
        alpha = rdotr / (p.dot(Ap))
        x = x + alpha * p
        r = r - alpha * Ap
        newrdotr = r.dot(r)
        beta = newrdotr / rdotr
        p = r + beta * p

        if newrdotr < residual_tol:
            # Early CG termination because the residual is small
            break

    if callback is not None:
        callback(x)
    if verbose:
        obj_fn = 0.5 * x.dot(f_Ax(x)) - 0.5 * b.dot(x)
        norm_x = torch.norm(x) if type(x) is torch.Tensor else np.linalg.norm(x)
        print(fmtstr % (i, r.dot(r), norm_x, obj_fn))
    return x
