...
class BC:
    ...
    def mle_loss(self, data, idx):
        ...
        obs = data["observations"][idx].to(self.device)
        act = data["expert_actions"][idx].to(self.device)
    ...
    def fit(self, data, suppress_fit_tqdm=False, **kwargs):
        ...
        if self.save_logs:
            ...
            self.logger.log("loss_before", loss_val)
        ...
        # log stats after
        if self.save_logs:
            self.logger.log("epoch", self.epochs)
            loss_val = self.loss(data, idx=range(num_samples)).to("cpu").data.numpy().ravel()[0]
            self.logger.log("loss_after", loss_val)
            self.logger.log("time", (timer.time() - ts))

    def train(self, **kwargs):
        observations = np.concatenate([path["observations"] for path in self.expert_paths])
        observations = scale_obs(observations, self.obs_scale)
        expert_actions = np.concatenate([path["actions"] for path in self.expert_paths])
        observations = tensorize(observations, device="cpu")
        expert_actions = tensorize(expert_actions, device="cpu")
